# AWS経験者のためのAzure実践入門【サービス公開編・改訂版】
...
1. **基礎4章(旧1〜4章)を基礎編へ移管し、新章4本(Redis/メッセージング/External ID/APIM)を追加**:章数は17章と旧版(18章)並みだが、重複が消えた分すべてワークロード構築に使える。想定分量350〜400p。
2. **VM/App Service/Functionsを第1章「選択肢の地図」に統合**:App Service詳説は社内DX編、Functions実践は第7章(キューコンシューマ)に分担させ、重複を避けつつ体験は残す。旧版でApp Serviceに1章割いていた読者価値は、シリーズ全体では社内DX編でより深く回収される。
